...
def cut_coin(image, grad = 50):
    ...
    for i, s in enumerate(stats):
        ...
        if 0.85 < height/width < 1.15:
            ...
            parameters = [s[1], s[1]+height, s[0], s[0]+width]
    ...
    # MDL
    new_image_with_border = image[parameters[0]:parameters[1],parameters[2]:parameters[3]]
    max_x, max_y = len(new_image_with_border), len(new_image_with_border[0])
...
def get_features(coin, S,perimeter,mdl, rmax, rmin) -> dict:
    ...
    for i in start_pixels:
        for j in i:
            if j != [0,0,0]:
                pixels.append(j)
    features["MIN"] = min(pixels)
    features["MAX"] = max(pixels)
    n = len(pixels)
    b, g, r = 0, 0, 0
    for i in pixels:
        b += i[0]
...
d = [image_features(image_path) for image_path in images]
